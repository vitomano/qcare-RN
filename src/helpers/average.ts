...
export const average = (item:Status, pallets:DataPrereport[]):string => {
    let average = null
    if (item === "score") { average = pallets.map(sc => Number(sc.score || 0)) }
    if (item === "grade") { average = pallets.map(sc => Number(sc.grade || 0)) }
    if (item === "action") { average = pallets.map(sc => Number(sc.action || 0)) }
